...
// FILE: GetMotorcycleInteractor.cs
...
namespace Clean.UseCase.Interactors
{
    ...
    /// <summary>   A fetch motorcycle interactor. This class cannot be inherited. </summary>
    /// <remarks> 
    ///    TITLE
    ...
    /// </remarks>
    public sealed class GetMotorcycleInteractor
    {
        #region Properties
        ...
        /// </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public async Task<(GetMotorcycleResponse response, IError error)> HandleAsync(GetMotorcycleRequest request)
        {
            // Verify that the user has been properly authenticated.
            if (!_authService.IsAuthenticated())
            {
                return GetMotorcycleResponse.NewGetMotorcycleResponse(null,
                                                                            OperationStatus.NotAuthenticated,
                                                                            new Error("Get operation failed due to not being authenticated."));
            }
            ...
            if (!_authService.IsAuthorized(AuthorizationRole.Admin))
            {
                return GetMotorcycleResponse.NewGetMotorcycleResponse(null,
                                                                            ...
                                                                            new
                                                                                Error("Get operation failed due to not being authorized, so please contact your system administrator."));
            }

            // Get the motorcycle with Id from the repository.
            (Motorcycle motorcycle, OperationStatus status, IError error) = await _motorcycleRepository.FetchByIdAsync(request.Id);

            if (error != null)
            {
                return GetMotorcycleResponse.NewGetMotorcycleResponse(null, status, error);
            }
            ...
            return GetMotorcycleResponse.NewGetMotorcycleResponse(motorcycle, OperationStatus.Ok, null);
        }
        ...
        ///
        /// <returns>
        /// Returns  (null, error) when there is an error, otherwise (GetMotorcycleInteractor, null).
        /// </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public static (GetMotorcycleInteractor interactor, IError error) NewGetMotorcycleInteractor(
            IMotorcycleRepository motorcycleRepository,
            IAuthService authService)
        {
            var interactor = new GetMotorcycleInteractor
            {
